    ...
    methods: {
        allProducts(){
            ...
            this.$store.dispatch("getReq", {
                ...
                params: {
                    date: null,
                    user_id: this.$session.get('su') && Number(this.$session.get('warehouseId'))==this.$session.get('userId') ? 0 : this.$session.get('warehouseId')
                    // user_id: this.$session.get('su') ? 0 : this.$session.get('warehouseId')
